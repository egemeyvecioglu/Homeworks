...
def convolution(x0,s0,x1,s1,N):
    ...
    for q in range(len(newlist)):
        newlist[q] += s0
    ax3.stem(newlist[:170],conv[:170],markerfmt=' ')
    ax3.set_title("x[n] * h[n]")
    plt.xlabel('n')
    plt.ylabel('Amplitude')
    fig1.suptitle(f'Convolution of the signal x[n] with {N} point average filter h[n]')
    plt.savefig(f"7b_convolution{N}.pdf", format="pdf", bbox_inches="tight")
    # plt.show()
...
if __name__ == "__main__":
    ...
    for N in [3, 5, 10 ,20]:
        ...
        convolution(data0,s0,data1,s0,N)