...
# 750 "/Applications/microchip/xc8/v2.30/pic/include/pic18_chip_select.inc" 3
# 1 "/Applications/microchip/xc8/v2.30/pic/include/proc/pic18f4620.inc" 1 3
# 48 "/Applications/microchip/xc8/v2.30/pic/include/proc/pic18f4620.inc" 3
PORTA equ 0F80h
...
TOSU equ 0FFFh
# 4576 "/Applications/microchip/xc8/v2.30/pic/include/proc/pic18f4620.inc" 3
...
# 751 "/Applications/microchip/xc8/v2.30/pic/include/pic18_chip_select.inc" 2 3
...
;Work done by Group 62 : Hasan Ege Meyvecio?lu and Mert Cenk Yediparmak
...
interrupt_service_routine:
    ...
    btfsc INTCON, 0 ; Check ((INTCON) and 0FFh), 0, a, skip if clear
    goto rb_interrupt
...
rb_interrupt:
    ...
    btfss temp_portb, 4 ;Check if ((PORTB) and 0FFh), 4, a is pressed.
    call rb4_interrupt

    btfss temp_portb, 5 ;Check if ((PORTB) and 0FFh), 5, a is pressed.
    call rb5_interrupt

    btfss temp_portb, 6 ;Check if ((PORTB) and 0FFh), 6, a is pressed.
    call rb6_interrupt

    btfss temp_portb, 7 ;Check if ((PORTB) and 0FFh), 7, a is pressed.
    call rb7_interrupt
...
rb4_interrupt: ;start/stop button
    bcf INTCON, 0 ;clear ((INTCON) and 0FFh), 0, a
...
start_mode:
    movlw 00000011B ; Start with lightning ((PORTC) and 0FFh), 0, a-1
    movwf LATC
...
timer0_interrupt:
    ...
    movlw 00000001B
    movwf temp_latc ;Light ((PORTC) and 0FFh), 0, a

    movlw 1
    subwf current_beat, 0
    btfsc STATUS, 2
    bsf temp_latc, 1 ; If current beat = 1, then a new bar starts. Light ((PORTC) and 0FFh), 1, a
...
main:
    ...
    ; Enable global and peripheral interrupts by setting ((INTCON) and 0FFh), 7, a and ((INTCON) and 0FFh), 6, a bits to 1
    clrf INTCON
    bsf INTCON,3 ; Enable BIT3: ((INTCON) and 0FFh), 3, a
    bsf INTCON,5 ; Enable BIT5: ((INTCON) and 0FFh), 5, a
    bsf INTCON,6 ; Enable BIT6:((INTCON) and 0FFh), 6, a
    bsf INTCON,7 ; Enable BIT7:((INTCON) and 0FFh), 7, a
...
display_ra0:
    movlw 1B
    movwf LATA ;Select ((PORTA) and 0FFh), 0, a
    clrf WREG ; W = 0
    btfsc pause_mode,0
...
display_ra1:
    movlw 10B
    movwf LATA ;Select ((PORTA) and 0FFh), 1, a
    clrf LATD
    clrf WREG ; W = 0
    btfss pause_mode,0
...
display_ra2:
    movlw 100B
    movwf LATA ;Select ((PORTA) and 0FFh), 2, a
    clrf LATD
    bsf LATA, 2
    clrf WREG ; W = 0
    btfss pause_mode,0
...
display_ra3:
    movlw 1000B
    movwf LATA ;Select ((PORTA) and 0FFh), 3, a
    clrf LATD
    clrf WREG ; W = 0
...
start_timers:
    ...
    ; Bit 6 ((T0CON) and 0FFh), 6, a = 0, 16 bit configuration
    ; Bit 7 ((T0CON) and 0FFh), 7, a = 1, enable timer0 now.
    ; Bit 3 ((T0CON) and 0FFh), 3, a = 0, prescaler will be used.
    ; Bits 2:0 ((T0CON) and 0FFh), 2, a:0 = 011, 1:16 prescaling
    movlw 10000011B
    movwf T0CON

    ;Enable Timer1 with no prescaling and 16 bit mode and ((T1CON) and 0FFh), 6, a = 1
    movlw 11001001B
    movwf T1CON
